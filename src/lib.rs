...
pub async fn handle_client(
    ...
    let welcome_message = format!("{} joined the chat!\n", my_username);
    let _ = sender.send((my_username.clone(), welcome_message.clone()));
    write_stream.lock().await.write_all(welcome_message.as_bytes()).await.unwrap();
	...
	println!("DEBUG: Broadcasting disconnect message: {}", disconnect_message); // Debug log
